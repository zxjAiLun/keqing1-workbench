...
// 三页（天梯/账号/模型）统一展示的天梯数据更新状态 footer（R11 post-release
// UX cleanup：不暴露 snapshot_id 等内部术语）。
//
// 展示：数据更新 <本地时间> · 已计入 <games> 场 · 可见时每 30s 自动刷新
// 状态表现：
// - 首次加载：不显示（season 数据尚未就绪）；
// - 静默轮询中：显示"正在检查更新…"；
// - 成功：恢复普通文字；
// - 轮询失败：继续显示上一份有效数据，不弹整页错误。
...
export function LadderSnapshotStatus({ season, refreshing }: LadderSnapshotStatusProps) {
  ...
  return (
    <div style={containerStyle} aria-live="polite">
      {refreshing ? '正在检查更新…' : (
        <>
          数据更新 {fmtUpdatedAt(season.updated_at)} · 已计入 {season.games ?? '—'} 场
